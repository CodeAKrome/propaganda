...
class BiasResult:
    ...
    def to_text(self) -> str:
        ...
        return (
            ...
            f"Degree: {self.dominant_degree} ({self.degree_percent[self.dominant_degree_code]}%)\n"
...
class BiasEngine:
    ...
    def analyze(self, text: str) -> BiasResult:
        ...
        if validated:
            # Calculate confidence based on dominant score
            dir_values = list(validated["dir"].values())
            deg_values = list(validated["deg"].values())
            dir_confidence = max(dir_values) - (sum(dir_values) / len(dir_values))
            deg_confidence = max(deg_values) - (sum(deg_values) / len(deg_values))
            confidence = (dir_confidence + deg_confidence) / 2
