...
def get_model(model_path: str, base_model_name: str):
    ...
    if model_instance is None:
        ...
        model_instance = {"model": model, "tokenizer": tokenizer, "device": device}
...
async def predict(request: PredictRequest):
    ...
    try:
        ...
        instance = get_model(
            request.model_path or "./bias-detector-output", request.base_model_name or "t5-large"
        )

        return PredictResponse(result=result, device=instance["device"])
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("T5_PORT", "1337"))
    print(f"Starting Bias Detector API on device: {get_device()} at port {port}")
