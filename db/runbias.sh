...
# Get timestamp date for fallback
timestamp_date=$(cat ../db/timestamp.txt 2>/dev/null | cut -d'T' -f1)

# Check if ids.txt is empty or doesn't exist
if [[ ! -s "ids.txt" ]]; then
    echo "ids.txt is empty or missing, using timestamp date: $timestamp_date"
    ./geminize.py --dst bias --queryfile ../llm/prompt/claudeopus_CoVe.xml --start-date "$timestamp_date" --ollama --model gpt-oss:120b
else
    echo "ids.txt exists with content, using IDs from file"
    ./geminize.py --dst bias --queryfile ../llm/prompt/claudeopus_CoVe.xml --idsource ids.txt --ollama --model gpt-oss:120b
fi
