...
def cypher(pairs, svo_prompt, vec_file, cypher_file):
    ...
    while remaining_pairs:
        ...
        print(f"Debug: exit_code={exit_code}, file exists={file_exists}, size={file_size} bytes", file=sys.stderr)
        
        if exit_code == 0 and file_size > 0:
            print(f"Cypher succeeded with {src}: {model}")
            return True
        else:
            print(f"Cypher failed with {src}: {model} (exit_code: {exit_code})", file=sys.stderr)
            
            # Remove failed pair, but keep if it's the last one
            if len(remaining_pairs) > 2:
                print(f"Removing failed model {src}: {model} from retry list", file=sys.stderr)
                remaining_pairs = remaining_pairs[2:]
            else:
...
def report(pairs, reporter_prompt, cypher_file, reporter_file, vec_file, news_file, query):
    ...
    while remaining_pairs:
        ...
        if exit_code == 0 and file_size > 0:
            ...
            print(f"Report failed with {src}: {model} (exit_code: {exit_code}, output empty: {output_empty})", file=sys.stderr)
