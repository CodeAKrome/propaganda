...
def clean_article_text(article: str) -> tuple[str, dict]:
    ...
    was_changed = (
        stats["single_word_lines_removed"] > 0
        or stats["double_newlines_replaced"] > 0
        or cleaned_text != article
    )
...
def process_articles(
    coll, limit: int | None = None, dry_run: bool = False, batch_size: int = BATCH_SIZE
):
    """
    ...
    for doc in cursor:
        ...
        if len(batch) >= batch_size:
            ...
            batch_result = process_batch(
                batch, coll, dry_run, report, max_sample_changes=5
            )
            processed += len(batch)
            batch = []
            ...
            # Progress update
            progress = (
                (processed / total_to_process) * 100 if total_to_process > 0 else 0
            )
            print(
                f"Progress: {processed}/{total_to_process} ({progress:.1f}%) - Modified: {report['documents_modified']}"
            )
...
def process_batch(batch, coll, dry_run, report, max_sample_changes=5):
    ...
    for doc in batch:
        ...
        if stats["unchanged"]:
            ...
            report["total_single_word_lines_removed"] += stats[
                "single_word_lines_removed"
            ]
            report["total_double_newlines_replaced"] += stats[
                "double_newlines_replaced"
            ]
            ...
            if not dry_run:
                try:
                    coll.update_one(
                        {"_id": doc_id}, {"$set": {"article": cleaned_article}}
                    )
                except Exception as e:
                    report["errors"].append({"doc_id": str(doc_id), "error": str(e)})
...
def print_report(report: dict):
    ...
    print(f"\n🧹 CLEANING STATISTICS:")
    print(
        f"   Single-word lines removed:       {report['total_single_word_lines_removed']:,}"
    )
    print(
        f"   Double newlines replaced:        {report['total_double_newlines_replaced']:,}"
    )
    ...
    if report["sample_changes"]:
        ...
        for i, change in enumerate(report["sample_changes"], 1):
            ...
            print(
                f"      - Single-word lines removed: {change['single_word_lines_removed']}"
            )
            print(
                f"      - Double newlines replaced:   {change['double_newlines_replaced']}"
            )
            print(
                f"      - Size: {change['original_length']:,} → {change['new_length']:,} chars"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean article text")
    parser.add_argument(
        "--limit",
        "-l",
        type=int,
    ...
    parser.add_argument(
        "--uri", type=str, default=MONGO_URI, help=f"MongoDB URI (default: {MONGO_URI})"
    )
    ...
    report = process_articles(
        coll, limit=args.limit, dry_run=args.dry_run, batch_size=args.batch_size
    )
