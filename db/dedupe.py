...
def find_repeated_lines(articles):
    ...
    repeated_lines = {
        norm_line
        for norm_line, article_ids in line_to_articles.items()
...
def process_source(collection, source, safe_mode=True):
    ...
    if repeated_lines:
        ...
        sorted_lines = sorted(
            repeated_lines, key=lambda x: len(line_to_articles[x]), reverse=True
        )
        ...
        if removed_count > 0:
            ...
            if not safe_mode:
                collection.update_one(
                    {"_id": article_id}, {"$set": {"article": cleaned_text}}
                )
...
def main():
    ...
    print("=" * 80)
    print(
        f"Mode: {'SAFE MODE (no changes will be made)' if args.safe else 'LIVE MODE (will modify database)'}"
    )
    ...
    print(f"\nBreakdown by source:")
    print(
        f"{'Source':<40} {'Articles':<12} {'Modified':<12} {'Lines Removed':<15} {'Repeated Patterns'}"
    )
    print("-" * 100)

    for result in sorted(results, key=lambda x: x["total_lines_removed"], reverse=True):
        print(
            f"{result['source']:<40} {result['total_articles']:<12} "
