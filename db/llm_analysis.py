    ...
    try:
        ...
        for text in texts:
            response = requests.post(
                "http://localhost:1337/predict", json={"text": text[:5000]}, timeout=60
            )
                ...
                results.append(
                    {
                        "direction": result.get(
                            "direction",
                            result.get("dir", {"L": 0.33, "C": 0.34, "R": 0.33}),
                        ),
                        "degree": result.get(
                            "degree",
                            result.get("deg", {"L": 0.33, "M": 0.34, "H": 0.33}),
                        ),
    ...
    for text in texts:
        ...
        if total == 0:
            ...
            if left_pct > right_pct and left_pct > center_pct:
                direction = {
                    "L": round(left_pct * 0.8 + 0.1, 2),
                    "C": round(center_pct * 0.5, 2),
                    "R": round(max(0, 1 - left_pct * 0.8 - center_pct * 0.5 - 0.1), 2),
                }
            elif right_pct > left_pct and right_pct > center_pct:
                direction = {
                    "L": round(max(0, 1 - right_pct * 0.8 - center_pct * 0.5 - 0.1), 2),
                    "C": round(center_pct * 0.5, 2),
                    "R": round(right_pct * 0.8 + 0.1, 2),
                }
            else:
                direction = {
                    "L": round(left_pct, 2),
                    "C": round(center_pct + 0.1, 2),
                    "R": round(right_pct, 2),
                }
...
def write_output(records: List[Dict], output_path: str, format: str = "jsonl"):
    ...
    if format == "jsonl":
        ...
        # Pretty JSON
        output_records = [
            {k: v for k, v in r.items() if k != "entities_raw"} for r in records
        ]
        with open(output_path, "w") as f:
...
def main():
    ...
    parser.add_argument(
        "--text", type=str, help="Full-text search query on article field"
    )
    parser.add_argument(
        "--skip-existing-bias",
        action="store_true",
    ...
    if texts_to_analyze:
        ...
        for i in range(0, len(texts_to_analyze), BATCH_SIZE):
            ...
            print(
                f"  Processed {min(i + BATCH_SIZE, len(texts_to_analyze))}/{len(texts_to_analyze)}"
            )
