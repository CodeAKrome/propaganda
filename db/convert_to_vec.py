...
def connect_to_mongo():
    """Initialize MongoDB connection using MONGO_URI environment variable."""
...
def fetch_article_data(collection, article_id):
    ...
    try:
        ...
        if doc is None:
            print(
                f"Warning: Document with ID '{article_id}' not found", file=sys.stderr
            )
            return None
    ...
    for category in data["categories"]:
        ...
        with open(output_file, "w") as outf:
            ...
            write_category_header(
                outf, category_name, category_id, category["article_count"]
            )
        ...
        print(
            f"  Articles: {category_processed} processed, {category_skipped} skipped\n"
        )
