...
def main(
    ...
    try:
        ...
        logger.info(
            f"Generating embeddings for article titles (batch_size={batch_size})"
        )
        titles = articles_df["title"].tolist()
        embeddings = generate_embeddings(titles, batch_size=batch_size)
        logger.info(f"Generated embeddings with shape {embeddings.shape}")

        # Step 3: Initialize ChromaDB
        logger.info(f"Initializing ChromaDB in {persist_dir}")
        collection = initialize_chroma(
            collection_name=config.COLLECTION_NAME, persist_dir=persist_dir
        )
        ...
        categories = generate_category_names(
            articles_df, cluster_labels, min_cluster_size
        )
        ...
        for cat_id, cat_data in results["categories"].items():
            print(
                f'Category {cat_id}: "{cat_data["category_name"]}" '
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=f"Batch size for embeddings (default: {config.EMBEDDING_BATCH_SIZE})",
    )
