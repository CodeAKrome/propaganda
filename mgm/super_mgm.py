...
Creates narrated news clips with:
  ...
  - Enhanced Stable-Diffusion backgrounds with comprehensive entity awareness
  - Procedural portraits, banners, transitions
  - Context-aware background generation from structured input
...
# Enhanced Helpers
# ------------------------------------------------------------------
def parse_structured_input(path: str) -> list:
    """Parse the new structured input format with <text>, <background>, and <entities> sections."""
    ...
    for line in lines:
        ...
        # Parse <text> sections
        if line.startswith('<text>') and line.endswith('</text>'):
            current_text = line[6:-7]  # Extract content between tags
        
        # Parse <background> sections
        elif line.startswith('<background>') and line.endswith('</background>'):
            current_background = line[12:-13]
        
        # Parse <entities> sections
        elif line.startswith('<entities>') and line.endswith('</entities>'):
            entities_content = line[10:-11]
            current_entities = {}
            
            if entities_content:
                # Parse entity tags like <person>Name</person>
                entity_pattern = r'<(\w+)>([^<]+)</\1>'
                matches = re.findall(entity_pattern, entities_content)
                
                for entity_type, entity_value in matches:
                    entity_type_upper = entity_type.upper()
                    if entity_type_upper not in current_entities:
                        current_entities[entity_type_upper] = []
                    
                    # Split comma-separated entities
                    entity_values = [e.strip() for e in entity_value.split(',')]
                    current_entities[entity_type_upper].extend(entity_values)
            
            # When we have all three components, create a segment
            if current_text is not None:
                segments.append({
                    'text': current_text,
                    'background_prompt': current_background if current_background else '',
                    'entities': current_entities,
                    'people': current_entities.get('PERSON', []),
                    'places': current_entities.get('GPE', []) + current_entities.get('LOC', []),
                    'topics': [],  # Will be extracted from text
                    'sentiment': 'neutral'  # Will be analyzed from text
                })
...
class EnhancedNewsVideoGenerator:
    ...
    def _setup_metal(self):
        if platform.system() == "Darwin" and torch.backends.mps.is_available():
            ...
            print("⚠️ Using CPU", file=sys.stderr)
    ...
    def _build_focused_entity_prompt(self, segment: dict, txt_entities: dict = None) -> str:
        """
        Create ONE prompt focused on the provided background prompt or entities.
        """
        # First, use the pre-generated background prompt if available
        if segment.get('background_prompt') and segment['background_prompt'].strip():
            return f"cinematic digital art, {segment['background_prompt']}, highly detailed, 4K, professional news quality"
        
        # Fallback to entity-based prompt generation
        if not txt_entities and not segment.get('entities'):
            return self._fallback_prompt(segment)

        # Use segment entities if available, otherwise use txt_entities
        entities_to_use = segment.get('entities', {}) if segment.get('entities') else txt_entities
        found = self._find_all_entities_in_sentence(segment["text"], entities_to_use) if txt_entities else segment.get('entities', {})
    ...
    def generate_audio(self, text: str, output_path: Path) -> float:
        ...
        if self.device.type in ("mps", "cuda"):
            try:
                ...
                print(f"⚠️ GPU TTS failed, fallback CPU: {e}", file=sys.stderr)
                generator = self.pipeline(text, voice=self.voice)
        ...
        if not valid:
            print("⚠️ No audio generated, creating 0.1 s silence", file=sys.stderr)
        ...
        if not output_path.exists() or output_path.stat().st_size < 1024:
            print("⚠️ Audio file empty, overwriting with 0.1 s silence", file=sys.stderr)
    ...
    def generate_enhanced_video(self, segments: list, output_video_path: str,
        ...
        for i, seg in enumerate(segments):
            ...
            seg['sentiment'] = self._analyze_sentiment(seg['text'])
            seg['topics'] = self._extract_topics(seg['text'])
            ...
            try:
                ...
                with AudioFileClip(str(audio_path)) as ac:
                    dur = ac.duration
                if dur < 0.1:
                    print(f"⚠️ Skipping segment {i+1} — audio too short ({dur:.2f}s)", file=sys.stderr)
                    continue
            except Exception as e:
                print(f"⚠️ Error generating audio for segment {i+1}, skipping: {e}", file=sys.stderr)
                continue

            # Now that we have the exact duration, build the video segment with enhanced backgrounds
            try:
                # Generate background for this segment
                ...
                print(f"⚠️ Error creating video segment {i+1}, skipping: {e}", file=sys.stderr)
                continue
...
# ------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Enhanced local news-video generator with structured input")
    parser.add_argument("input", help="Structured input file with <text>, <background>, and <entities> sections")
    ...
    # Parse structured input file
    print(f"Parsing structured input from {args.input}...", file=sys.stderr)
    segments = parse_structured_input(args.input)
